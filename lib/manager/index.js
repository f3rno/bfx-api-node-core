...
const initWS = require('../ws2/init_state')
...
const onOrderCancelGroupEvent = require('../manager/events/order_cancel_group')
...
class Manager extends EventEmitter {
  ...
  /**
    * @return {number} channelsPerSocket
    */
  getChannelsPerSocket () {
    return this._channelsPerSocket
  }

  /**
    * Update default calc & dms for all future sockets
    *
    * @param {Object} args
    * @param {number?} args.calc - default 0
    * @param {number?} args.dms - default 0, active 4
    */
  updateAuthArgs (args = {}) {
    this.authArgs = {
      ...this.authArgs,
  ...
  /**
   * Authenticates all open non-authenticated API connections
   *
   ...
   * @param {number?} args.calc
   * @return {Promise} p - resolves once all sockets have authenticated, rejects on any error
   */
  async auth ({ apiKey, apiSecret, dms, calc } = {}) {
    ...
    return PI.forEach(this.wsPool, async (ws, i) => {
  ...
  async closeAllSockets () {
    for (let i = this.wsPool.length - 1; i >= 0; i -= 1) {
      await this.closeWS(this.wsPool[i].id)
    }
  }
  ...
  openWS () {
    ...
    ev.on('exec:order:cancel:group', onOrderCancelGroupEvent.bind(this, this, id))
  ...
  /**
   * Binds a handler to an event mapped to a specific socket in the pool, and
   * updates the socket with the handler's return value if an object.
   *
   ...
   * updates the socket with the handler's return value if an object.
   *
   * The handler is discarded after being invoked once.
   *
   * @param {string} eventName
   * @param {Object} filterValue - value(s) to check for
   * @param {Method} cb
  ...
  notifyPlugins (type, section, name, args = {}) {
    Object.values(this.plugins)
      ...
      .forEach(p => {
        this.applyPlugin(p, section, name, args)
      })
  }

  /**
    * Calls a plugin handler with the provided arguments and each socket in
    * the matching pool (only ws2 plugins apply), optionally updating each
    * socket state if applicable.
    *
    * @param {Object} plugin - whole plugin instance
    * @param {string} section - section on plugin to notify
    * @param {string} name - name of handler on section to notify
    * @param {Object} args - passed to plugin handler along with each socket
    */
  applyPlugin (plugin, section, name, args = {}) {
  ...
  withDataSocket (filterCb, cb) {
    const wsI = this.wsPool.findIndex(ws => {
      ...
      return _some(Object.values(channels), filterCb)
    })
  ...
  /**
    * Returns the current socket pool size
    *
    * @return {number} size
    */
  getWSPoolSize () {
    return this.wsPool.length
  }
}
